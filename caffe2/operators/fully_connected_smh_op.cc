...
#include "caffe2/operators/fully_connected_smh_op.h"

namespace caffe2 {

REGISTER_CPU_OPERATOR(FCSmh, FullyConnectedSmhOp<CPUContext>);
REGISTER_CPU_OPERATOR(FCSmhGradient, FullyConnectedSmhGradientOp<CPUContext>);

OPERATOR_SCHEMA(FCSmh)
    .NumInputs(3)
    .NumOutputs(1)
...
OPERATOR_SCHEMA(FCSmhGradient).NumInputs(3).NumOutputs(2, 3);

class GetFCSmhGradient : public GradientMakerBase {
...
REGISTER_GRADIENT(FCSmh, GetFCSmhGradient);
}  // namespace caffe2
